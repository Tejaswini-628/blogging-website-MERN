...
import serviceAccountKey from "./react-js-blog-website-yt-70c26-firebase-adminsdk-ao2f5-495140c1bf.json" assert {type:"json"};
...
server.post("/signin",(req,res)=>{
    ...
    .then((user) => {
        ...
        bcrypt.compare(password, user.personal_info.password,(err,result)=>{
            if(err){
                return res.status(403).json({"error":"Error occcured while login please try again"});
            }
...
server.post("/change-password", verifyJWT,  (req,res)=> {
    ...
    if(!passwordRegex.test(currentPassword) || !passwordRegex.test(newPassword)){
        return  res.status(403).json({ error:"Passwords must be at least 6 to 20 characters long with a numeric, 1 lowercase and 1 uppercase letters" })
    }
    ...
    .then((user) => {
        ...
        bcrypt.compare(currentPassword, user.personal_info.password, (err,result) => {
            if(err){
                return res.status(500).json({ error: "Some error occured while changing the password, pleases try again later"})
            }
...
//         let {_id:asd ,comment, commentedAt, children} = commentFile;

//         Blog.findOneAndUpdate({_id}, { $push : { "comments": commentFile._id}, $inc: {"activity.total_comments": 1 ,"activity.total_parent_comments": replying_to ? 0 : 1 }}, { new: true })
//         .then(blog => {
...
server.post("/add-comment", verifyJWT, async (req, res) => {
    try {
        ...
        const { _id: commentId, commentedAt, children } = commentFile;

        // Update blog comments and activity
